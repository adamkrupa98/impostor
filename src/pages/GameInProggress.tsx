...
const GameInProggress = () => {
  ...
  const lastPlaye = currentIndex === roles.length - 1;
  const currentPlayer = roles[currentIndex];
    ...
    <div className='w-full h-full flex items-center justify-center'>
      <div className='relative flex flex-col items-center w-[40%] h-[80%] bg-gray-800 rounded-xl shadow-lg p-3 border-[3px] border-purple-700'>
          <p className='text-3xl font-semibold text-center'>{roles[currentIndex].name}</p>
          ...
          <div className={`capitalize flex border-2  w-[50%] mt-5 p-3 rounded-md items-center justify-center ${roles[currentIndex].role == 'impostor' ? 'border-red-500' : 'border-green-400'}`}>
            {
          ...
          <div className='absolute bottom-20 flex justify-between w-[90%]'>
            
           <button className={buttonClass} onClick={()=> setCurrentIndex(prev => prev -1 )}>cofnij</button>
